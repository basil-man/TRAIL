...
class ReplayBuffer:
        ...
        if reward is not None:
            ...
            if len(self.all_rewards) >= 10:  # 至少有10个样本才开始比较
                ...
                if sample_reward < reward_threshold:
                    message = (f"Sample rejected: reward {sample_reward:.4f} not in top {self.top_percent*100}% "
    ...
    def sample(self, batch_size: int) -> List[DataProto]:
        ...
        if len(self.buffer) <= batch_size:
            ...
            if len(samples) == 1:
                ...
                samples = samples * 2
                print(f"警告: 仅有1个样本，复制后变为{len(samples)}个")
                
            # 如果是奇数条样本，补充一个副本确保为偶数，有利于均匀分布到多个GPU
            if len(samples) % 2 == 1:
                samples.append(samples[0])
                print(f"警告: 样本数为奇数，添加1个副本使其变为偶数: {len(samples)}")
            ...
            if len(indices) % 2 == 1:
                ...
                if remaining_indices:  # 如果还有可用样本
                    ...
                    print(f"采样到奇数{len(indices)-1}个样本，额外采样1个使其变为偶数: {len(indices)}")
                else:  # 如果没有更多样本可用，复制一个已有的样本
                    indices = np.append(indices, indices[0])
                    print(f"采样到奇数{len(indices)-1}个样本，复制1个使其变为偶数: {len(indices)}")
            ...
            print(f"采样完成: 随机采样了{len(samples)}个样本")
    ...
    def should_distill(self, current_step: int, reward_mean: float = None) -> bool:
        ...
        if steps_since_last_distill >= self.fixed_distill_interval:
            print(f"达到固定蒸馏间隔 {self.fixed_distill_interval} 步，触发蒸馏")
            return True
        
        # 记录当前状态供调试
        if steps_since_last_distill % 5 == 0:  # 每5步记录一次，避免日志过多
            print(f"蒸馏状态: 步数={steps_since_last_distill}, 固定间隔={self.fixed_distill_interval}, "
                  f"填充率={len(self.buffer) / self.capacity:.2f}")
    ...
    def update_distill_metrics(self, current_step: int, reward_improvement: float = 0.0):
        ...
        # 打印蒸馏结果
        print(f"\n===== 蒸馏执行结果 =====")
        print(f"步骤: {current_step}, 奖励改进: {reward_improvement:.4f}")
        if len(self.avg_rewards) >= 2:
            print(f"奖励变化: {reward_diff:.4f}")
        print(f"下次蒸馏间隔: {self.fixed_distill_interval} (固定)")
        print(f"缓冲区大小: {len(self.buffer)}/{self.capacity}")
        print(f"当前阈值比例: {self.distill_threshold_ratio:.2f}")
    ...
    def get_distill_stats(self) -> Dict[str, Any]:
        ...
        base_interval_factor = max(0.5, 1.0 - (steps_since_last_distill / self.max_distill_interval)) if self.max_distill_interval > 0 else 1.0
        fullness_factor = min(1.0, buffer_fullness_ratio / self.distill_threshold_ratio) if self.distill_threshold_ratio > 0 else 1.0
        ...
        epsilon = 1e-8
        dynamic_threshold = self.distill_threshold_ratio * base_interval_factor * history_factor / (fullness_factor * quality_factor + epsilon)
        ...
        stats = {
            ...
            # 历史改进信息
            "avg_recent_improvement": np.mean(self.reward_improvement[-3:]) if len(self.reward_improvement) >= 3 else 0.0,
            "total_distillations": len(self.reward_improvement),
            
            # 蒸馏状态 - 更新同样的阈值
            "should_distill_soon": buffer_fullness_ratio >= dynamic_threshold or 
                                  (avg_buffer_reward > 0.95 and len(self.buffer) >= 10),
            ...
            "steps_until_next_distill": self.fixed_distill_interval - (steps_since_last_distill if hasattr(self, 'steps_since_last_distill') else 0),
        }
