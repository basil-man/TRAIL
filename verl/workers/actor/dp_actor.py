    ...
    def update_actor_off_policy(self, data: DataProto):
        ...
        if is_weights is not None:
            ...
            if is_weights.shape != advantages.shape:
                ...
                if len(is_weights.shape) == len(advantages.shape) and is_weights.shape[1] == advantages.shape[1]:
                    # 重新调整is_weights以匹配当前批次大小
                    batch_ratio = advantages.shape[0] / is_weights.shape[0]
                    if batch_ratio > 1:  # 需要扩展is_weights
                        # 如果是整数倍，直接repeat，否则按需复制
                        if batch_ratio == int(batch_ratio):
                            is_weights = is_weights.repeat(int(batch_ratio), 1)
                        else:
                            # 处理非整数倍情况
                            needed = advantages.shape[0]
