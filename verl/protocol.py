...
def union_tensor_dict(tensor_dict1: TensorDict, tensor_dict2: TensorDict, override=False) -> TensorDict:
    """Union two tensordicts.
    
    Args:
        tensor_dict1: First TensorDict
        tensor_dict2: Second TensorDict
        override: If True, values from tensor_dict2 will override values in tensor_dict1 for keys that exist in both
    """
    ...
    for key in tensor_dict2.keys():
        if key not in tensor_dict1.keys():
            ...
            if not override:
                ...
                # Override the value in tensor_dict1 with the value from tensor_dict2
                tensor_dict1[key] = tensor_dict2[key]
...
class DataProto:
    ...
    def select_by_indices(self, indices) -> "DataProto":
        """
        根据索引选择 DataProto 的子集。

        Args:
            indices (list or tensor): 要从批次中选择的索引

        Returns:
            DataProto: 一个只包含所选索引的新 DataProto
        """
        ...
        # 将 torch 索引转换为 numpy 索引，用于 non_tensor_batch
        ...
        # Add this method to the DataProto class
    @classmethod
    def from_list(cls, data_list: List["DataProto"]) -> "DataProto":
        """Create a DataProto from a list of DataProto objects.
        
        Args:
            data_list (List[DataProto]): List of DataProto objects to combine
            
        Returns:
            DataProto: A new DataProto that combines all the input DataProtos
        """
        if not data_list:
            return cls()
            
        # For DataProtoItem objects, convert them to DataProto first
        converted_list = []
        for item in data_list:
            if isinstance(item, DataProtoItem):
                # Convert DataProtoItem to DataProto
                tensors = {key: val.unsqueeze(0) for key, val in item.batch.items()} if item.batch is not None else {}
                non_tensors = {key: np.array([val], dtype=object) for key, val in item.non_tensor_batch.items()}
                converted_list.append(cls(
                    batch=TensorDict(source=tensors, batch_size=(1,)) if tensors else None,
        ...
        # Use the existing concat method to combine all DataProto objects
        return cls.concat(converted_list)
    ...
    def union(self, other: "DataProto", override=True) -> "DataProto":
        """Union with another DataProto. Union batch and meta_info separately.
        
        Args:
            other (DataProto): another DataProto to union
            override (bool): If True, values from other will override values in self for keys that exist in both
        
        Returns:
            DataProto: the DataProto after union
        """
        self.batch = union_tensor_dict(self.batch, other.batch, override=override)
    ...
    def groupby(self, group_key: str) -> Dict[Any, "DataProto"]:
        """
        This function groups the batch by a key, which must be one of the keys in the batch. Then it returns a list of sliced-DataProto.

        Args:
            group_key (str): name of the key
        """

        assert (group_key in self.batch.keys()) or (
            group_key in self.non_tensor_batch.keys()
        ), f"{group_key} not in the batch keys"

        if group_key in self.batch.keys():
            unique_key, group_indices = self.batch[group_key].unique(
                return_inverse=True
            )
            unique_key = unique_key.tolist()
        else:
            unique_key, group_indices = np.unique(
                self.non_tensor_batch[group_key], return_inverse=True
            )
            unique_key = unique_key.tolist()

        result = []
        for k in range(len(unique_key)):
            indices = np.where(group_indices == k)[0]
            if len(indices) != 0:
                result.append(
                    DataProto.from_dict(
                        tensors={
                            key: val[torch.tensor(indices)]
                            for key, val in self.batch.items()
                        },
                        non_tensors={
                            key: val[np.array(indices)]
                            for key, val in self.non_tensor_batch.items()
                        },
                        meta_info=self.meta_info,
                    )
