...
def compute_score(
    ...
    with lock:
        if (answer, ground_truth) in results_cache:
            return results_cache[(answer, ground_truth)], answer

    output_queue = multiprocessing.Queue()
    process = multiprocessing.Process(
        target=math_equal_process, args=(answer, ground_truth, output_queue)
    )
    process.start()
    process.join(10)
    ...
    if process.is_alive():
        ...
        try:
            acc = output_queue.get(timeout=1)
...
def compute_score_lingua(
    solution_str, ground_truth, results_cache, lock, format_score=0.0, 
    analyzer_api_url="http://localhost:5000/get_token_importance",lam=0.8
) -> tuple:
    answer = extract_answer(solution_str, "math", use_last_number=False)
    if answer is None or answer == "":
        return 0, answer
    ...
    try:
        ...
        importance=lam+importance*(1-lam)
