    ...
    def _perform_distillation(self, timing_raw: dict):
        ...
        with marked_timer("distillation", timing_raw, color="purple"):
            ...
            for i, microbatch_samples in enumerate(replay_samples_list):
                ...
                microbatch = DataProto.from_list(microbatch_samples)
                ...
                current_log_prob = self.actor_rollout_wg.compute_log_prob(microbatch)
                microbatch = microbatch.union(current_log_prob)

                # Compute importance sampling weights
                old_log_probs = microbatch.batch["log_prob_old"]
                new_log_probs = microbatch.batch["old_log_probs"]
                max_is_weight = replay_config.get("max_is_weight", 10.0)
                is_weights = torch.exp(new_log_probs.sum(-1) - old_log_probs.sum(-1))
                    ...
                    if self.use_replay_buffer:
                        with marked_timer("add_to_buffer", timing_raw, color="orange"):
                            for i in range(len(batch.batch)):
                                sample_data = batch.select_by_index(i)
                                self.replay_buffer.add(sample_data, sample_data.batch["old_log_probs"])
