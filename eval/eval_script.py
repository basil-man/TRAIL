...
def eval_amc(item, pred_key='prediction', prec=1e-3):
    pred = item[pred_key]
    if pred_key == 'program_output' and isinstance(pred, str):
        pred = [pred]
    ans = item['answer']
    ...
    return is_correct_amc(item, pred_key=pred_key, prec=prec)

def is_correct_amc(item, pred_key='prediction', prec=1e-3):
    pred = item[pred_key][0]
    ans = item['answer']
    # print("pred:",pred)
    # print("ans:",ans)
    if isinstance(pred, list) and isinstance(ans, list):
